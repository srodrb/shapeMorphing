...
class airfoil {
public:
    airfoil (float chord,float M, float P, float T, int nodes);
    ~airfoil () {};
...
airfoil::airfoil(float chord,float M, float P, float T,int nodes)
{
    m = 0.01*M;
    p = 0.1*P;
    t = 0.01*T;
...
void airfoil::info()
{
    //cout << "Naca airfoil: " << m << "," << "," << p << "," << t << " alfa: " << alpha << endl;
}

void airfoil::coordinates()
{
    ...
    if(p == 0 or m == 0){
        //debug_printf("-----Symmetric NACA 4 digits airfoil-----\n");
        for (int i = 0; i < Ni; i++) {
            xu[i] =     x[i];
            xl[i] =     x[i];
            zu[i] =     z_t[i];
        ...
        //debug_printf("-----Non symmetric NACA 4 digits airfoil-----\n");
        double z_c[Ni];
        double theta[Ni];
        for (int i = 0; i < Ni; i++) {
            if(x[i] <= p*c)
    ...
    //bucle para imprimir cosas...
    //debug_printf("-----Showing xu-----\n");
    for (int i = 0; i < Ni; i++) {
        //debug_printf("xu[%d] = %.6f\n", i+1,xu[i]);
    }
    //debug_printf("-----Showing zu-----\n");
    for (int i = 0; i < Ni; i++) {
        //debug_printf("zu[%d] = %.6f\n", i+1,zu[i]);
    }
    //debug_printf("-----Showing xl-----\n");
    for (int i = 0; i < Ni; i++) {
        //debug_printf("xl[%d] = %.6f\n", i+1,xl[i]);
    }
    //debug_printf("-----Showing zl-----\n");
    for (int i = 0; i < Ni; i++) {
        //debug_printf("zl[%d] = %.6f\n", i+1,zl[i]);
    }
}

void airfoil::AOA(float angle)
{
    /*
     * Aplicamos el giro a las coordenadas del perfil para situarlo al angulo de ataque especificado.
     */
    alpha = (angle*pi)/180.0;
    for (int i = 0; i < Ni; i++) {
        xu[i] = xu[i]*cos(alpha) + zu[i]*sin(alpha);
        zu[i] = -xu[i]*sin(alpha) + zu[i]*cos(alpha);
        xl[i] = xl[i]*cos(alpha) + zl[i]*sin(alpha);
        zl[i] = -xl[i]*sin(alpha) + zl[i]*cos(alpha);
        cout << "zu[" << i+1 << "]: " << zu[i] << endl;
    }

    cout << "Mostramos xu tras el giro: " << endl;
    for (int i = 0; i < Ni; i++) {
        cout << "xu[" << i+1 << "]: " << xu[i] << endl;
    }
    cout << "Mostramos zu tras el giro: " << endl;
    for (int i = 0; i < Ni; i++) {
    cout << "zu[" << i+1 << "]: " << zu[i] << endl;
    }
    cout << "Mostramos xl tras el giro: " << endl;
    for (int i = 0; i < Ni; i++) {
    cout << "xl[" << i+1 << "]: " << xl[i] << endl;
    }
    cout << "Mostramos zl tras el giro: " << endl;
    for (int i = 0; i < Ni; i++) {
    cout << "zl[" << i+1 << "]: " << zl[i] << endl;
    }

}

void airfoil::meshGen()
{
...
void airfoil::outPlot()
{
    ...
    int output = system ("gnuplot plotAirfoil.gp");
    if( output != 0)
    {
...
void airfoil::exportCoordinates(const char* filename)
{
    FILE *out;
