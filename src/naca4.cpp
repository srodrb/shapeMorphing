...
void naca4::meshGen()
{
    ...
    out = fopen(setPath("constant/polyMesh","blockMeshDict"),"w");
...
void naca4::sampleDictGen()
{
    FILE *out;
    out = fopen(setPath("system","sampleDict"),"w");
    ...
    fprintf(out, "    format      ascii;                                                          \n");
    fprintf(out, "    class       dictionary;                                                     \n");
    fprintf(out, "    object      sampleDict;                                                     \n");
    fprintf(out, "}                                                                               \n");
    fprintf(out, "// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * // \n");
    fprintf(out, "\n");


    fprintf(out, "setFormat raw;\n");
    fprintf(out, "interpolationScheme cellPoint;\n");
    fprintf(out, "fields ( p );\n");  
    fprintf(out, "sets\n");
    fprintf(out, "(\n"); 

    fprintf(out, "      upper_surf {\n"); 
    fprintf(out, "                 type cloud;\n"); 
    fprintf(out, "                 axis xyz;\n"); 
    fprintf(out, "                 points (\n");

    for (int i = 0; i < Ni; ++i)
    {
    fprintf(out, "                          (%f 0.000000 %f)\n", xu[i],zu[i]);// spline 4-5
    }
    fprintf(out, "                          (1.000000 0.000000 0.000000)\n");// add end point (not really necessary)
    fprintf(out, "                         ); \n");
    //fprintf(out, "                  maxDistance 0.01;\n");
    //fprintf(out, "                  patches ("airfoil");\n");
    fprintf(out, "                   } \n");

    fprintf(out, "      lower_surf {\n"); 
    fprintf(out, "                 type cloud;\n"); 
    fprintf(out, "                 axis xyz;\n"); 
    fprintf(out, "                 points (\n");
